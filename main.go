...
//*********************
var TotalAmount int
var TotalBurnAmount int
var BalanceArray = make(map[string]int)
var CarryArray = make(map[string]int)
var isBlockMined bool
...
func pickWinner() {
	...
	OUTER:
		for _, block := range temp {
			...
			var CarryTemp []int
			for addr := range CarryArray {
				CarryTemp = append(CarryTemp, CarryArray[addr])
			}
			fmt.Printf("Carry Forward Array : ")
			fmt.Println(CarryTemp)
			for addr := range validators {
				validators[addr] += CarryArray[addr]
			}
			mutex.Lock()
			...
			CarryArray = validators

			for addr := range CarryArray {
				CarryArray[addr] = int(float64(CarryArray[addr]) * 0.8)
			}
...
func handleConn(conn net.Conn) {
	...
	io.WriteString(conn, "Enter Amount to burn :")
	valid_miner := bufio.NewScanner(conn)
	for valid_miner.Scan() {
		burnAmount, err := strconv.Atoi(valid_miner.Text())
		mutex.Lock()
		fmt.Printf("Burn Amount " + string(burnAmount) + "\n")
		mutex.Unlock()
		if err != nil {
		...
		// if burnAmount < BalanceArray[address] {
		validators[address] = burnAmount
		BalanceArray[address] = 100 - burnAmount
		if BalanceArray[address] < 0 {
			log.Printf("insufficient Balance, Enter burn amount less than 100")
			return
		}
		break
